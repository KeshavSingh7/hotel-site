$(document).ready(function() {
  ...
  var day = d.getDate();
  var cur = (month<10 ? '0' : '') + month + '/' + (day<10 ? '0' : '') + day + '/' + d.getFullYear();
  var x = $('#checkin');
  var y = $('#checkout');
  var w = $('#adults');
  ...
  $(".btn-primary").click(function(){
    if(x.val() >= cur && x.val().length > 0) {
        if(y.val() > x.val() && y.length > 0) {
          ...
          alert('checkout field cannot be empty');
        else
          alert('check out date cannot be same as or less than check in date');
    }
    else if (x.val() == 0)
      alert('checkin field cannot be empty');
